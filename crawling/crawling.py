    ...
    while True:
        ...
        for i in range(10):  
            try:
                ...
                # file_name = f"./data/{dic}/competition_rate/경쟁률_{home_name}.csv"
                # df.to_csv(file_name, index=False, encoding="utf-8-sig")
                # print(f"데이터가 '{file_name}' 파일로 저장되었습니다.")
...
def housing_application_announcement_download(url,dic):
    DOWNLOAD_DIR = f"./data/{dic}/housing_application_announcement"
            ...
            try:
                ...
                if pdf_down_button:
                    ...
                    if title_element:
                        ...
                        file_name = f"{title}_모집공고.pdf"

                        file_path = os.path.join(DOWNLOAD_DIR, file_name)

                        if os.path.exists(file_path):
                            print(f"이미 존재하는 파일입니다. 다운로드를 건너뜁니다: {file_path}")
                            print("다운로드를 종료합니다.")
                            return  # 함수 종료
                            ...
                            if response.status_code == 200:
                                ...
                                print(f"PDF 다운로드 실패: {pdf_url}")
                    else:
                        print("모집공고 제목을 찾을 수 없습니다.")
