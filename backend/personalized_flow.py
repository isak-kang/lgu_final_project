...
FLOW_STEPS = {
    ...
    "L5_14": 8,  # 세대구성원 소득
    "L5_15": 9,  # 소득세 납부여부
    "L5_2": 10, "L5_16": 10, "L5_17": 10, "L5_18": 10, "L5_19": 10, "L5_20": 10, "L5_21": 10  # 결과 확인
}
...
class UserState:
    ...
    def determine_apply_type(self):
      ...
      income_list = self.income_list
      num_family = self.num_family
      income_level = self.determine_income_level()
      num_family = self.num_family

      apply_type = []
      # 신혼부부 맞벌이
      if ("L5_3" in scenario_logs) and ("L5_9" in scenario_logs):
        if income_level <= 2 and income_list[0]*10000 <= 3482964:
        ...
        elif income_level <= 5 and income_list[0]*10000 <= int(round(3482964*1.4, 0)):
          if "L5_6" in scenario_logs:
            apply_type.append({
              "type" : "신혼부부 특별공급",
...
def return_text(apply_type:List):
...
"""
  if len(apply_type) >= 2:
    for types in apply_type[:-1]:
      ...
      txt += f"""
{type_of_specialty}에서 {income_step},
"""
    txt += f"{apply_type[-1]['type']}에서 {apply_type[-1]['income_step'][1]}으로 신청이 가능해요."
    return txt
...
* 무주택세대구성원이란 세대에 속한 자(본인, 배우자, 본인의 직계존속인 부모와 조부모, 배우자의 직계존속, 본인의 자녀와 손자) 전원이 주택이나 분양권을 소유하고 있지 않은 세대의 구성원을 뜻합니다.""",
      buttons = [
...
자세한 사항은 부동산공시가격 알리미(https://www.realtyprice.k) 또는 
위택스(https://www.wetax.go.kr/main.do)를 확인해주세요.""",
      buttons = [
...
def update_content(apply_type):
  # print(f"전 : {SCENARIOS['L5_18']['content']}")
  SCENARIOS["L5_18"]["content"] = apply_type
  SCENARIOS['L5_19']["text"] = return_text(apply_type)
  print(f"후 : {SCENARIOS['L5_18']['content']}")
...
def get_personalized_response(question: str) -> Optional[FlowResponse]:
  ...
  if match_digit and user_state.current_scenario not in ["L5_0", "L5_16", "L5_17", "L5_18", "L5_19", "L5_20"]:
    ...
    # 세대구성원 수 묻기 
      ...
      user_state.update_scenario("L5_14")
      print(user_state.current_scenario)
    ...
    # 세대구성원의 소득 묻기 
      ...
      try:
        ...
        print(f"Error occurred while processing income: {e}")
      ...
      try:
        newly_bool = any(types.get('type') == "생애최초 특별공급" for types in apply_type)
        if newly_bool:
          update_content(apply_type)
          return SCENARIOS.get("L5_15")
      except Exception as e:
         print(f"Error occurred while processing income: {e}")
      
      update_content(apply_type)
      print(SCENARIOS["L5_18"]["content"])
